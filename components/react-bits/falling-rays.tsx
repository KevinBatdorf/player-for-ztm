...
const RayScene: React.FC<RaySceneProps> = ({
  ...
  useFrame((state) => {
    if (materialRef.current) {
      (materialRef.current.uniforms as any).uTime.value = state.clock.elapsedTime;
      (materialRef.current.uniforms as any).uResolution.value.set(
        size.width,
        size.height,
      );
      ...
      (materialRef.current.uniforms as any).uMotionBlur.value = motionBlur;
      (materialRef.current.uniforms as any).uBgGlow.value = bgGlow;
      (materialRef.current.uniforms as any).uColor1.value.set(color1);
      (materialRef.current.uniforms as any).uColor2.value.set(color2);
    }
  });
