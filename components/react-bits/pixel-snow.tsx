...
// Vendored shim: `uniforms` is an index signature, which this repo's
// noUncheckedIndexedAccess widens to possibly-undefined on every lookup.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const u = (m: ShaderMaterial): any => m.uniforms;
...
export default function PixelSnow({
  ...
  const handleResize = useCallback(() => {
    ...
    resizeTimeoutRef.current = window.setTimeout(() => {
      ...
      u(material).uResolution.value.set(w, h);
    }, 100);
  }, []);
  ...
  useEffect(() => {
    ...
    u(material).uFlakeSize.value = flakeSize;
    u(material).uMinFlakeSize.value = minFlakeSize;
    u(material).uPixelResolution.value = pixelResolution;
    u(material).uSpeed.value = speed;
    u(material).uDepthFade.value = depthFade;
    u(material).uFarPlane.value = farPlane;
    u(material).uBrightness.value = brightness;
    u(material).uGamma.value = gamma;
    u(material).uDensity.value = density;
    u(material).uVariant.value = variantValue;
    u(material).uDirection.value = (direction * Math.PI) / 180;
    u(material).uColor.value.copy(colorVector);
  }, [
    flakeSize,
    minFlakeSize,
    pixelResolution,
